...
subroutine STATISTICS_convdiff( time, dt, u, g, x0, dx, stage)
    ...
    case ("post_stage")
        ...
        if (params_convdiff%time_statistics) then
            ...
            params_convdiff%time_statistics_mean = params_convdiff%time_statistics_mean / get_active_domain_length(params_convdiff%domain_size, params_convdiff%domain_cropping_min, params_convdiff%domain_cropping_max, dir=merge('xyz', 'xy ', params_convdiff%dim==3))
