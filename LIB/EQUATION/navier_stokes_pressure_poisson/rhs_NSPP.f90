...
subroutine RHS_NSPP_Velocity(g, Bs, dx, x0, phi, order_discretization, time, rhs, mask, n_domain)
    ...
    if (params_nspp%HIT_linear_forcing) then
        ...
        e_kin_set = params_nspp%HIT_energy * get_active_domain_length(params_nspp%domain_size, params_nspp%domain_cropping_min, params_nspp%domain_cropping_max, dir=merge('xyz', 'xy ', params_nspp%dim==3))
